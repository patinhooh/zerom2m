...
static const char *kCreateResources =
...
static const char *kCreateAE =
    ...
    "ontologyRef TEXT, nl TEXT, csz TEXT, regs INTEGER, rr INTEGER, mei TEXT, tri TEXT, trn INTEGER, srv TEXT, esi TEXT, tren INTEGER);";

static const char *kCreateContainer =
    "CREATE TABLE IF NOT EXISTS container (ri TEXT PRIMARY KEY, st INTEGER, cr TEXT, mni INTEGER, mbs INTEGER, mbis INTEGER, mia INTEGER, cni INTEGER, cbs INTEGER, ontologyRef TEXT, disr INTEGER, li TEXT, loc TEXT);";

static const char *kCreateContentInstance =
    "CREATE TABLE IF NOT EXISTS content_instance (ri TEXT PRIMARY KEY, st INTEGER, cr TEXT, cnf TEXT, cs INTEGER, conr TEXT, con BLOB, ontologyRef TEXT, dgt TEXT, dcnt INTEGER);";

static const char *kCreateSubscription =
    "CREATE TABLE IF NOT EXISTS subscription (ri TEXT PRIMARY KEY, enc TEXT, exc INTEGER, nu TEXT, gpi TEXT, nfu TEXT, bn TEXT, rl TEXT, psn INTEGER, pn INTEGER, nsp INTEGER, ln INTEGER, nct INTEGER, nec INTEGER, su TEXT, cr TEXT, gn TEXT, acrs TEXT);";
...
bool Database::InitSchema(CString &err)
{
    if (!db_) { err = "DB not open"; return false; }
    char *zErr = nullptr;
    int rc;
    rc = sqlite3_exec(db_, kCreateResources, nullptr, nullptr, &zErr);
    if (rc != SQLITE_OK) { err = CString(zErr); sqlite3_free(zErr); return false; }
...
{
    if (!v.has_value() || v.value().GetLength() == 0)
...
bool Database::SavePrimitiveContent(const PrimitiveContent &pc, CString &err)
{
    ...
    if (rc != SQLITE_OK) { if (zErr) { err = CString(zErr); sqlite3_free(zErr); } return false; }
    ...
    auto insert_resources = [&](const ResourceBase &rbase)->bool {
        const char *sql = "INSERT OR REPLACE INTO resources (ri,rn,ty,pi,ct,lt,et,labels,acpi,custodian,daci,announceTo,announcedAttribute,announceSyncType) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
        ...
        sqlite3_bind_int(stmt, 3, static_cast<int>(rbase.resourceType.has_value() ? (int)rbase.resourceType.value() : 0));
        ...
        if (rbase.expirationTime.has_value()) bind_text_or_null(stmt, 7, rbase.expirationTime.value()); else sqlite3_bind_null(stmt,7);
        ...
        if (rbase.announceSyncType.has_value()) sqlite3_bind_int(stmt, 14, rbase.announceSyncType.value()); else sqlite3_bind_null(stmt,14);

        rc = sqlite3_step(stmt);
        sqlite3_finalize(stmt);
        if (rc != SQLITE_DONE) { err = CString(sqlite3_errmsg(db_)); return false; }
        return true;
    };
    ...
    switch (pc.kind()) {
        case PrimitiveContentKind::CSEBase: {
            ...
            if (!r) { ok = false; break; }
            if (!insert_resources(*r)) { ok = false; break; }
            const char *sql = "INSERT OR REPLACE INTO ae (ri, api, aei) VALUES (?,?,?);"; // reuse ae schema for cse minimal fields
            sqlite3_stmt *stmt = nullptr;
            int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
            if (rc != SQLITE_OK) { err = CString(sqlite3_errmsg(db_)); ok = false; break; }
            ...
            rc = sqlite3_step(stmt);
            sqlite3_finalize(stmt);
            if (rc != SQLITE_DONE) { err = CString(sqlite3_errmsg(db_)); ok = false; break; }
            break;
        }
        case PrimitiveContentKind::AE: {
            ...
            if (!r) { ok = false; break; }
            if (!insert_resources(*r)) { ok = false; break; }
            const char *sql = "INSERT OR REPLACE INTO ae (ri, api, aei, apn, poa, ontologyRef, nl, csz, regs, rr, mei, tri, trn, srv, esi, tren) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
            sqlite3_stmt *stmt = nullptr;
            int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
            if (rc != SQLITE_OK) { err = CString(sqlite3_errmsg(db_)); ok = false; break; }
            bind_text_or_null(stmt,1,r->resourceID);
            ...
            if (r->registrationStatus.has_value()) sqlite3_bind_int(stmt,9,r->registrationStatus.value()); else sqlite3_bind_null(stmt,9);
            if (r->requestReachability.has_value()) sqlite3_bind_int(stmt,10,r->requestReachability.value()?1:0); else sqlite3_bind_null(stmt,10);
            bind_text_or_null(stmt,11,r->m2mExtID);
            bind_text_or_null(stmt,12,r->triggerRecipientID);
            if (r->triggerReferenceNumber.has_value()) sqlite3_bind_int(stmt,13,r->triggerReferenceNumber.value()); else sqlite3_bind_null(stmt,13);
            CString srv = VecToPacked(r->supportedReleaseVersions);
            bind_text_or_null(stmt,14,srv);
            bind_text_or_null(stmt,15,r->e2eSecInfo);
            if (r->triggerEnable.has_value()) sqlite3_bind_int(stmt,16,r->triggerEnable.value()?1:0); else sqlite3_bind_null(stmt,16);
            rc = sqlite3_step(stmt);
            sqlite3_finalize(stmt);
            if (rc != SQLITE_DONE) { err = CString(sqlite3_errmsg(db_)); ok = false; break; }
            break;
        ...
        case PrimitiveContentKind::Container: {
            ...
            if (!r) { ok = false; break; }
            if (!insert_resources(*r)) { ok = false; break; }
            const char *sql = "INSERT OR REPLACE INTO container (ri, st, cr, mni, mbs, mbis, mia, cni, cbs, ontologyRef, disr, li, loc) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?);";
            sqlite3_stmt *stmt = nullptr;
            int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
            if (rc != SQLITE_OK) { err = CString(sqlite3_errmsg(db_)); ok = false; break; }
            bind_text_or_null(stmt,1,r->resourceID);
            if (r->stateTag.has_value()) sqlite3_bind_int(stmt,2,r->stateTag.value()); else sqlite3_bind_null(stmt,2);
            bind_text_or_null(stmt,3,r->creator);
            if (r->maxNrOfInstances.has_value()) sqlite3_bind_int64(stmt,4,r->maxNrOfInstances.value()); else sqlite3_bind_null(stmt,4);
            if (r->maxByteSize.has_value()) sqlite3_bind_int64(stmt,5,r->maxByteSize.value()); else sqlite3_bind_null(stmt,5);
            if (r->maxByteSizePerInstance.has_value()) sqlite3_bind_int64(stmt,6,r->maxByteSizePerInstance.value()); else sqlite3_bind_null(stmt,6);
            if (r->maxInstanceAge.has_value()) sqlite3_bind_int64(stmt,7,r->maxInstanceAge.value()); else sqlite3_bind_null(stmt,7);
            ...
            if (r->disableRetrieval.has_value()) sqlite3_bind_int(stmt,11,r->disableRetrieval.value()?1:0); else sqlite3_bind_null(stmt,11);
        ...
        case PrimitiveContentKind::ContentInstance: {
            ...
            if (!r) { ok = false; break; }
            if (!insert_resources(*r)) { ok = false; break; }
            const char *sql = "INSERT OR REPLACE INTO content_instance (ri, st, cr, cnf, cs, conr, con, ontologyRef, dgt, dcnt) VALUES (?,?,?,?,?,?,?,?,?,?);";
            sqlite3_stmt *stmt = nullptr;
            int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
            if (rc != SQLITE_OK) { err = CString(sqlite3_errmsg(db_)); ok = false; break; }
            bind_text_or_null(stmt,1,r->resourceID);
            ...
            if (r->deletionCnt.has_value()) sqlite3_bind_int(stmt,10,r->deletionCnt.value()); else sqlite3_bind_null(stmt,10);
            rc = sqlite3_step(stmt);
            sqlite3_finalize(stmt);
            if (rc != SQLITE_DONE) { err = CString(sqlite3_errmsg(db_)); ok = false; break; }
            break;
        ...
        case PrimitiveContentKind::Subscription: {
            ...
            if (!r) { ok = false; break; }
            if (!insert_resources(*r)) { ok = false; break; }
            const char *sql = "INSERT OR REPLACE INTO subscription (ri, enc, exc, nu, gpi, nfu, bn, rl, psn, pn, nsp, ln, nct, nec, su, cr, gn, acrs) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
            sqlite3_stmt *stmt = nullptr;
            int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
            if (rc != SQLITE_OK) { err = CString(sqlite3_errmsg(db_)); ok = false; break; }
            bind_text_or_null(stmt,1,r->resourceID);
            ...
            bind_text_or_null(stmt,2,CString());
            if (r->expirationCounter.has_value()) sqlite3_bind_int(stmt,3,r->expirationCounter.value()); else sqlite3_bind_null(stmt,3);
            ...
            if (r->preSubscriptionNotify.has_value()) sqlite3_bind_int(stmt,9,r->preSubscriptionNotify.value()); else sqlite3_bind_null(stmt,9);
            if (r->pendingNotification.has_value()) sqlite3_bind_int(stmt,10,r->pendingNotification.value()); else sqlite3_bind_null(stmt,10);
            if (r->notificationStoragePriority.has_value()) sqlite3_bind_int(stmt,11,r->notificationStoragePriority.value()); else sqlite3_bind_null(stmt,11);
            if (r->latestNotify.has_value()) sqlite3_bind_int(stmt,12,r->latestNotify.value()?1:0); else sqlite3_bind_null(stmt,12);
            if (r->notificationContentType.has_value()) sqlite3_bind_int(stmt,13,static_cast<int>(r->notificationContentType.value())); else sqlite3_bind_null(stmt,13);
            if (r->notificationEventCat.has_value()) sqlite3_bind_int(stmt,14,r->notificationEventCat.value()); else sqlite3_bind_null(stmt,14);
...
// Minimal loader: find resource by ri, rn or pi/rn and populate PrimitiveContent for supported types
bool Database::LoadPrimitiveContentByTarget(const CString &target, PrimitiveContent &out, CString &err)
{
    if (!db_) { err = "DB not open"; return false; }

    const char *sql = "SELECT ri, rn, ty, pi, ct, lt, et, labels FROM resources WHERE ri = ? OR rn = ? OR (pi || '/' || rn) = ? LIMIT 1;";
    sqlite3_stmt *stmt = nullptr;
    int rc = sqlite3_prepare_v2(db_, sql, -1, &stmt, nullptr);
    if (rc != SQLITE_OK) { err = CString(sqlite3_errmsg(db_)); return false; }
    ...
    rc = sqlite3_step(stmt);
    if (rc != SQLITE_ROW) { sqlite3_finalize(stmt); err = "not found"; return false; }
    ...
    switch (static_cast<zerom2m::onem2m::types::ResourceType>(ty)) {
        case ResourceType::AE: {
            const char *q = "SELECT api, aei, apn, poa, ontologyRef, nl FROM ae WHERE ri = ? LIMIT 1;";
            sqlite3_stmt *s2 = nullptr;
            int rc2 = sqlite3_prepare_v2(db_, q, -1, &s2, nullptr);
            if (rc2 != SQLITE_OK) { err = CString(sqlite3_errmsg(db_)); return false; }
        ...
        case ResourceType::Container: {
            const char *q = "SELECT st, cr, mni, mbs, mbis, mia, cni, cbs, ontologyRef, disr, li, loc FROM container WHERE ri = ? LIMIT 1;";
            sqlite3_stmt *s2 = nullptr;
            int rc2 = sqlite3_prepare_v2(db_, q, -1, &s2, nullptr);
            if (rc2 != SQLITE_OK) { err = CString(sqlite3_errmsg(db_)); return false; }
            sqlite3_bind_text(s2,1,ri.c_str(),-1,SQLITE_TRANSIENT);
            ...
            if (rc2 == SQLITE_ROW) {
                ...
                if (sqlite3_column_type(s2,0) != SQLITE_NULL) c.stateTag = (u32)sqlite3_column_int(s2,0);
                c.creator = CString(reinterpret_cast<const char*>(sqlite3_column_text(s2,1)));
                if (sqlite3_column_type(s2,2) != SQLITE_NULL) c.maxNrOfInstances = (s64)sqlite3_column_int64(s2,2);
                if (sqlite3_column_type(s2,3) != SQLITE_NULL) c.maxByteSize = (s64)sqlite3_column_int64(s2,3);
                if (sqlite3_column_type(s2,4) != SQLITE_NULL) c.maxByteSizePerInstance = (s64)sqlite3_column_int64(s2,4);
                if (sqlite3_column_type(s2,5) != SQLITE_NULL) c.maxInstanceAge = (s64)sqlite3_column_int64(s2,5);
                ...
                if (sqlite3_column_type(s2,9) != SQLITE_NULL) c.disableRetrieval = (sqlite3_column_int(s2,9) != 0);
        ...
        case ResourceType::ContentInstance: {
            const char *q = "SELECT st, cr, cnf, cs, conr, con, ontologyRef, dgt, dcnt FROM content_instance WHERE ri = ? LIMIT 1;";
            sqlite3_stmt *s2 = nullptr;
            int rc2 = sqlite3_prepare_v2(db_, q, -1, &s2, nullptr);
            if (rc2 != SQLITE_OK) { err = CString(sqlite3_errmsg(db_)); return false; }
            sqlite3_bind_text(s2,1,ri.c_str(),-1,SQLITE_TRANSIENT);
            ...
            if (rc2 == SQLITE_ROW) {
                ...
                if (sqlite3_column_type(s2,0) != SQLITE_NULL) ci.stateTag = (u32)sqlite3_column_int(s2,0);
                ci.creator = CString(reinterpret_cast<const char*>(sqlite3_column_text(s2,1)));
                ci.contentInfo = CString(reinterpret_cast<const char*>(sqlite3_column_text(s2,2)));
                ...
                ci.ontologyRef = CString(reinterpret_cast<const char*>(sqlite3_column_text(s2,6)));
                ci.dataGenerationTime = CString(reinterpret_cast<const char*>(sqlite3_column_text(s2,7)));
                if (sqlite3_column_type(s2,8) != SQLITE_NULL) ci.deletionCnt = (u32)sqlite3_column_int(s2,8);
                out = ci;
                sqlite3_finalize(s2);
                return true;
            }
...
bool Database::GetCSEBase(CSEBase &out, CString &err)
{
    ...
    if (auto c = pc.GetIf<CSEBase>()) { out = *c; return true; }
