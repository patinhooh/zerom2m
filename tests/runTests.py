...
# Wrap the default TextTestRunner to inject a custom result class that
# forwards test outcomes to the reporter. Many test modules call
# `unittest.TextTestRunner(...).run(suite)` directly, so we monkeypatch
# TextTestRunner here before any module.run() is invoked.
_OrigTextTestRunner = _unittest.TextTestRunner


class _ReporterTextResult(_unittest.TextTestResult):
	def addSuccess(self, test):
		try:
			...
			# If reporter still has a current test, finalize it, otherwise update existing record
			if getattr(reporter, '_current_test', None) is not None and reporter._current_test.name == name:
				reporter.end_test('pass', '', '')
			else:
				reporter.update_test_status(name, module, 'pass', '', '')
	...
	def addFailure(self, test, err):
		try:
			...
			name = getattr(test, '_testMethodName', None)
			module = getattr(test.__class__, '__module__', '')
			tb = ''.join(_traceback.format_exception(*err)) if err else ''
			msg = ''
			try:
				msg = str(err[1]) if err and len(err) > 1 and err[1] is not None else ''
			except Exception:
				msg = ''
			if getattr(reporter, '_current_test', None) is not None and reporter._current_test.name == name:
				reporter.end_test('fail', msg, tb)
			else:
				reporter.update_test_status(name, module, 'fail', msg, tb)
	...
	def addError(self, test, err):
		try:
			...
			try:
				...
				msg = ''
			if getattr(reporter, '_current_test', None) is not None and reporter._current_test.name == name:
				reporter.end_test('error', msg, tb)
			else:
				reporter.update_test_status(name, module, 'error', msg, tb)
	...
	def addSkip(self, test, reason):
		try:
			...
			msg = str(reason)
			if getattr(reporter, '_current_test', None) is not None and reporter._current_test.name == name:
				reporter.end_test('skip', msg, '')
			else:
				reporter.update_test_status(name, module, 'skip', msg, '')


def _patched_TextTestRunner(**kwargs):
	# Ensure our resultclass is used while preserving other kwargs
	return _OrigTextTestRunner(resultclass=_ReporterTextResult, **kwargs)


# Monkeypatch unittest.TextTestRunner globally for imported test modules
_unittest.TextTestRunner = _patched_TextTestRunner
...
if __name__ == '__main__':
	...
	parser.add_argument('--disable-uppertester', '-nout', action='store_true', dest='disableUpperTester', default=True, help='disable the use of the upper tester interface')
		...
		if hasattr(module, 'run'):
			...
			if isRunTest(module): 	# exclude / include some tests
				for n in range(args.numberOfRuns):
					...
					# inform init/reporting about the running module
					init.set_current_module(module.__name__)
