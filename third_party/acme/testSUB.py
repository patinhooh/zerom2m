...
class TestSUB(unittest.TestCase):
	...
	def setUpClass(cls) -> None:
		global aeRN, aeURL, aePOAURL, cntRN, cntURL, subRN
		...
		# Create AE with POA (used by structured-NU and blocking-update tests)
		dct = { 'm2m:ae' : {
					'rn'  : f'{aeRN}POA',
					'api' : APPID,
					'rr'  : True,
		...
		aePoa, rsc = CREATE(cseURL, 'C', T.AE, dct)
		assert rsc == RC.CREATED, 'cannot create AE with poa'
		cls.originatorPoa = findXPath(aePoa, 'm2m:ae/aei')
		cls.aePOAURL = aePOAURL

		# Create AE without POA (used by test_createSUBwithUnknownPoa)
		dct = { 'm2m:ae' : {
					'rn'  : f'{aeRN}NoPOA',
					'api' : APPID,
					'rr'  : True,
		...
		cls.aeNoPoa, rsc = CREATE(cseURL, 'C', T.AE, dct)
		assert rsc == RC.CREATED, 'cannot create AE without poa'

		# Create CNT under the main AE
		dct = { 'm2m:cnt' : { 'rn' : cntRN }}
	...
	def test_createSUB(self) -> None:
		...
		self.assertEqual(findXPath(lastNotification, 'm2m:sgn/cr'), TestSUB.originator)
	...
	def test_createSUBForBatchNotificationNumber(self) -> None:
		...
		dct = { 'm2m:sub' : {
					...
					'nu': [ NOTIFICATIONSERVER ],
					# No su! bc we want to receive the last notification of a batch
					'bn': {
	...
	def test_createSUBBatchNotificationNumberWithLn(self) -> None:
		""" CREATE <SUB> with batch notification set to number and lastNotification=True -> Send verification notification"""
	...
	def test_createSUBWithEncChty(self) -> None:
		""" CREATE <SUB> to monitor creation of child resources with type=container -> Send verification notification"""
		self.assertIsNotNone(TestSUB.ae)
		self.assertIsNotNone(TestSUB.cnt)
	...
	def test_createAESUBwithOriginatorPOA(self) -> None:
		...
		self.assertEqual(rsc, RC.CREATED)
		ae2Originator = findXPath(ae, 'm2m:ae/aei')
		ae2URL = f'{cseURL}/{aeName}'
		clearLastNotification()
		...
		r, rsc = CREATE(ae2URL, ae2Originator, T.SUB, dct)
	...
	def test_createAESUBwithOriginatorPOANotReachable(self) -> None:
		...
		lastNotification = getLastNotification()	# no delay! blocking
		self.assertIsNone(findXPath(lastNotification, 'm2m:sgn/vrq'))
		# Create the CNT — should NOT trigger a notification (not request reachable)
		clearLastNotification()
		cntName = f'{cntRN}_{self._testMethodName}'
		dct = { 'm2m:cnt' : { 'rn' : cntName }}
		_, rsc = CREATE(ae2URL, ae2Originator, T.CNT, dct)
		self.assertEqual(rsc, RC.CREATED)
	...
	def test_createCNTwithOriginatorPOA(self) -> None:
		...
		cntName = f'{cntRN}_{self._testMethodName}'
		dct = { 'm2m:cnt' : { 'rn' : cntName }}
		_, rsc = CREATE(localURL, localOriginator, T.CNT, dct)
		self.assertEqual(rsc, RC.CREATED)
		lastNotification = getLastNotification(wait = notificationDelay)
	...
	def test_createSUBwithUnknownPoa(self) -> None:
		""" CREATE new <SUB> with NU pointing to an AE with no POA -> Fail """
		clearLastNotification()
		dct = { 'm2m:sub' : {
					'rn' : f'{subRN}NOPOA',
					'enc': {
		...
		_, rsc = CREATE(aeURL, TestSUB.originator, T.SUB, dct)
		self.assertEqual(rsc, RC.SUBSCRIPTION_VERIFICATION_INITIATION_FAILED)
	...
	def test_createSUBWithCreatorNull(self) -> None:
		...
		r, rsc = CREATE(cntURL, TestSUB.originator, T.SUB, dct)
		self.assertEqual(rsc, RC.CREATED)
		self.assertEqual(findXPath(r, 'm2m:sub/cr'), TestSUB.originator)	# creator should be set to originator
		...
		# Verify creator is present on RETRIEVE
		r, rsc = RETRIEVE(f'{cntURL}/{rn}', TestSUB.originator)
		self.assertEqual(rsc, RC.OK)
	...
	def test_createSubBlockingUpdate(self) -> None:
		...
		r, rsc = CREATE(TestSUB.aePOAURL, TestSUB.originatorPoa, T.SUB, dct)
		self.assertEqual(rsc, RC.CREATED, r)
...
def run(testFailFast:bool) -> TestResult:
	...
	addTests(suite, TestSUB, [
		...
		'test_createSUBwithUnknownPoa',		# was missing from suite
