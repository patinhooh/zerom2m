...
_current_module_name: str = ""   # set by runTest.py before each module run
...
def sendHttpRequest(method:str, url:str, originator:str, ty:ResourceTypes=None, data:JSON|str=None, ct:str=None, timeout:float=None, headers:Parameters=None) -> Tuple[STRING|JSON, int]:	# type: ignore # TODO Constants
	...
	reporter.record_request(
		...
		method       = method.upper(),
		url          = url,
		req_headers  = dict(hds),
		req_body     = data if not isinstance(data, bytes) else data.decode("utf-8", errors="replace"),
		resp_status  = rc,
		resp_headers = dict(r.headers),
		resp_body    = (r.json() if len(r.content) > 0
		                and (r.headers.get("Content-Type","").startswith(
		                    ("application/json","application/vnd.onem2m")))
		                else r.text if r.content else None),
	)
	# 
	...
	while True: 	# Timeout?
		...
		if message[0] == respTopic:
			...
			reporter.record_request(
				...
				req_headers  = {},
				req_body     = req,
				resp_status  = resp.get("rsc"),
				resp_headers = {k: v for k, v in resp.items() if k != "pc"},
				resp_body    = resp.get("pc"),
			)
		...
		if rvi == '5':
			if 'pc' in payload:
				reporter.record_request(
					...
					resp_status  = rsc,
					resp_headers = resp,
					resp_body    = payload if 'payload' in dir() else None,
				)
...
def testCaseStart(name:str) -> None:
	...
	"""
	reporter.begin_test(name, _current_module_name)  # <-- ADD
	if UPPERTESTERENABLED:
...
def testCaseEnd(name:str) -> None:
	"""	Indicate the end of a test case to the CSE via the UT interface.
		...
		Args:
			name: Name of the test case.
	"""
	# Determine test outcome by inspecting the calling TestCase (if any)
	import inspect
	import traceback
	status = "pass"
	failure_msg = ""
	...
	for frame_info in inspect.stack():
		...
		if 'self' in f_locals and isinstance(f_locals['self'], unittest.TestCase):
			_tc = f_locals['self']
			_outcome = getattr(_tc, '_outcome', None)
			if _outcome is not None:
				# failures and errors are lists of (test, exc_info)
				for entry in getattr(_outcome, 'failures', []) + getattr(_outcome, 'errors', []):
					if not entry:
						continue
					test_obj = entry[0]
					info = entry[1] if len(entry) > 1 else None
					# Match by method name or by test id ending
					...
					try:
						...
						pass
					matched = (method_name == name) or (test_id and test_id.endswith('.' + name))
					if matched:
						status = 'fail'
						# info may be a formatted traceback string or an exc_info tuple
						if isinstance(info, str):
							failure_msg = info.splitlines()[-1] if info else ''
							tb = info or ''
						else:
							try:
								failure_msg = str(info[1]) if info and len(info) > 1 and info[1] is not None else ''
						...
						break
				# skipped tests
				for test, reason in getattr(_outcome, 'skipped', []):
					if getattr(test, '_testMethodName', None) == name:
						status = 'skip'
						failure_msg = str(reason)
						break
			break

	reporter.end_test(status, failure_msg, tb)
	if UPPERTESTERENABLED:
